...
export const CustomizerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Load from Firestore always (real-time stream for both editor and visitor!)
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (data.siteSettings) {
          ...
          localStorage.setItem('salt_sea_custom_site', JSON.stringify(data.siteSettings));
        }
        if (data.themeSettings) {
          setThemeSettings(data.themeSettings);
          localStorage.setItem('salt_sea_custom_theme', JSON.stringify(data.themeSettings));
        }
        if (data.rooms) {
          setRooms(data.rooms);
          localStorage.setItem('salt_sea_custom_rooms', JSON.stringify(data.rooms));
        }
        if (data.galleryItems) {
          setGalleryItems(data.galleryItems);
          localStorage.setItem('salt_sea_custom_gallery', JSON.stringify(data.galleryItems));
        }
        if (data.sections) {
          setSections(data.sections);
          localStorage.setItem('salt_sea_custom_sections', JSON.stringify(data.sections));
        }
        if (data.bookings) {
          setBookings(data.bookings);
          localStorage.setItem('salt_sea_bookings', JSON.stringify(data.bookings));
        }
      } else {
        // Document does not exist yet. Seed Firestore with default values!
        ...
        setDoc(docRef, initialPayload)
          .then(() => {
            console.log("Firestore successfully seeded with default configurations!");
          })
          .catch((err) => {
      ...
      // Fallback to localStorage in case we are offline or Firestore fails
      ...
      if (savedTheme) try { setThemeSettings(JSON.parse(savedTheme)); } catch(e) {}
      if (savedRooms) try { setRooms(JSON.parse(savedRooms)); } catch(e) {}
      if (savedGallery) try { setGalleryItems(JSON.parse(savedGallery)); } catch(e) {}
      if (savedSections) try { setSections(JSON.parse(savedSections)); } catch(e) {}
      if (savedBookings) try { setBookings(JSON.parse(savedBookings)); } catch(e) {}
    });
  ...
  const updateSection = (id: string, updatedSection: Partial<PageSection>) => {
    setSections((prev) => {
      ...
      setDoc(doc(db, 'settings', 'config'), { sections: next }, { merge: true }).catch(err => console.error(err));
      return next;
    });
  };
  ...
  const reorderSections = (newSections: PageSection[]) => {
    ...
    setDoc(doc(db, 'settings', 'config'), { sections: newSections }, { merge: true }).catch(err => console.error(err));
  };
  ...
  const updateSiteSettings = (updates: Partial<SiteSettings>) => {
    setSiteSettings((prev) => {
      ...
      setDoc(doc(db, 'settings', 'config'), { siteSettings: next }, { merge: true }).catch(err => console.error(err));
      return next;
    });
  };
  ...
  const updateThemeSettings = (updates: Partial<ThemeSettings>) => {
    setThemeSettings((prev) => {
      ...
      setDoc(doc(db, 'settings', 'config'), { themeSettings: next }, { merge: true }).catch(err => console.error(err));
      return next;
    });
  };
  ...
  const updateRoom = (id: string, updatedRoom: Room) => {
    setRooms((prev) => {
      ...
      setDoc(doc(db, 'settings', 'config'), { rooms: next }, { merge: true }).catch(err => console.error(err));
      return next;
    });
  };
  ...
  const updateGalleryItem = (id: string, updatedItem: GalleryItem) => {
    setGalleryItems((prev) => {
      ...
      setDoc(doc(db, 'settings', 'config'), { galleryItems: next }, { merge: true }).catch(err => console.error(err));
      return next;
    });
  };
  ...
  const addBooking = (booking: Booking) => {
    setBookings((prev) => {
      ...
      setDoc(doc(db, 'settings', 'config'), { bookings: next }, { merge: true }).catch(err => console.error(err));
      return next;
    });
  };
